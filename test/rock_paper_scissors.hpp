...
struct Player {
    ...
    double pwin() {
        return (double) won / sum();
    }

    double ptie() {
        return (double) tie / sum();
    }

    double plos() {
        return (double) lost / sum();
    }
};
...
class RockPaperScissors : public Context {
public:
    int nb_game;
    Player p1, p2;
    ...
    p1(orps.p1), p2(orps.p2), p1_action(orps.p1_action), p2_action(orps.p2_action), nb_game(orps.nb_game) {
    }
    ...
    p1(_p1_bot), p2(_p2), p1_action(_p1_action), p2_action(_p2_action), nb_game(_nb_game) {
    }
    ...
    virtual Context* clone() const {
        return new RockPaperScissors(*this);
    }

    virtual vector<Context*> transition() {
        using namespace ActionType;
        vector<Context*> next(0);
        ...
        for (int action1 = 0; action1 < 3; ++action1)
            for (int action2 = 0; action2 < 3; ++action2) {
                ...
                next.push_back(new RockPaperScissors(p1n, p2n, (ActionType::Enum)action1, (ActionType::Enum)action2, nb_game + 1));
            }
        return next;
    }
